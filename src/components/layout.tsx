...
const blody = localFont({
  src: "../../public/BLOODY.ttf",
  variable: "--font-blody",
});
...
export function Layout({ children }: LayoutProps) {
  ...
  const isGame = router.pathname.includes("game");

  return (
    <div
      className={`flex flex-col text-black h-full bg-[#F0F0F0] ${blody.variable} ${inter.className}`}
    >
      <Head>
      ...
      <header
        className={twMerge(
          "flex-shrink-0 bg-primary pb-1.5 w-full overflow-hidden",
        )}
      >
        <div
          className={twMerge(
            "relative py-5 px-6 flex justify-end w-full",
            isGame ? "bg-white text-black" : "bg-primary text-white",
          )}
        >
          <h1 className="text-base leading-6 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
            Беспощадные ставки
          </h1>

          {isGame ? (
            <span className="whitespace-pre flex-shrink-0 h-6">
              {balance} L
            </span>
          ) : (
            <BellIcon />
      ...
      <main className="flex flex-col bg-[#F0F0F0] pb-10 h-full">
        {children}
      </main>

      <div className="z-[9999] text-3xl px-10 bg-primary-dark text-white absolute inset-0 hidden md:flex items-center justify-center">
        <span>Перейдите в мобильную версию сайта</span>
      </div>
    </div>
  );
}
